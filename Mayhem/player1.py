...
""" *Player1-class:
    ...
    -All of player1's variables are changed in the CONFIG-file
    -All of the methods in this class is updated in the "update" method
"""
class Player1(pygame.sprite.Sprite):
    def __init__(self, player1_bullets, x_pos, y_pos):
        pygame.sprite.Sprite.__init__(self)

        ########## CREATING PLAYER1-OBJECT TO ACCESS VARIABLES FROM CONFIG-FILE ##########
        self.p1 = Player1_traits()
        self.game_traits = Game_traits()
        self.bullet_traits = Bullet_traits()
    
        ########## TRAITS FOR THE SPRITE ##########        
        self.original_image = self.p1.PLAYER_IMAGE
        self.original_image = pygame.transform.scale(self.original_image, (self.p1.SIZE[0], self.p1.SIZE[1]))
        self.image = self.original_image
        self.rect = self.image.get_rect()
        self.rect.center = x_pos, y_pos

        self.speed = self.p1.PLAYER_SPEED
        self.fuel_tank = self.p1.FUEL_TANK
        self.fuel_consumption = self.p1.FUEL_CONSUMPTION
        self.angle = self.p1.ANGLE
        self.rotation_speed = self.p1.ROTATION_SPEED
    ...
    """
    def gravity(self):
        self.rect.y += self.p1.GRAVITY_EFFECT
    ...
    def fire(self):
        ...
        if self.now_time - self.start_time >= self.cooldown:
            if keys[pygame.K_m]:
                ...
                bullet = Bullet(self)
                self.player1_bullets.add(bullet)
